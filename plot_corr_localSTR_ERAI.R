...
pnum=1
pdf(file=fout,width=8,height=ss*2.7+0.8)
layout(cbind(c(seq(1,ss*2,2),ss*2+1),c(seq(2,ss*2,2),ss*2+1)),height=c(rep(1,ss),0.3))


par(mar=c(2,2,4,1))

 cyccorr<-array(NaN,c(length(lon),length(lat),2,ss,2))
...
for(i in 1:length(lon))
 ...
 image(lon,lat,cyccorr[,,1,s,1],breaks=breaks,col=col1,xlab="",ylab="",
          main=paste0("STRI corr: ",snames[s]))
   map('world2',add=T)
   contour(lon,lat,cyccorr[,,2,s,1],levels=c(-100,0.05,100),add=T,lwd=2,col="black",drawlabels=F)  

 image(lon,lat,cyccorr[,,1,s,2],breaks=breaks,col=col1,xlab="",ylab="",
          main=paste0("STRP corr: ",snames[s]))
   map('world2',add=T)
   contour(lon,lat,cyccorr[,,2,s,2],levels=c(-100,0.05,100),add=T,lwd=2,col="black",drawlabels=F) 
...
plot_indcorr_panel(1980,2016,seasons=smons,snames=slist,
       ...
       fout=paste0("paperfig_anticyccorr_localstr_ERAIpanel_proj100_rad10cv0.075_500km_2seasons.pdf"))


plot_indcorr_panel(1980,2016,seasons=smons2,snames=slist2,
       dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_highs_rad5cv0.075/",
       type="high",closed=T,move=500,
       fout=paste0("paperfig_anticyccorr_localstr_ERAIpanel_proj100_rad10cv0.075_500km_4seasons.pdf"))
