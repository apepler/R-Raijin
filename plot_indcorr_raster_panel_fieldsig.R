...
for(n in 1:3)
{
  ...
  for(s in 1:ss)
  {
   ...
   fid=fid+1
   tit=paste0(letters[fid],") ",snames[s]," correlation in ",reanals[n],": ",min(years2[I]),"-",max(years2[I]))
...
for(ind in c("SOI"))
{
plot_indcorr_panel(1910,2014,seasons=rbind(c(5,10),c(11,4)),snames=c("MJJASO","NDJFMA"),
       dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",iname=ind,
       type="anticyclone",proj="proj100_rad10cv0.075",type2="_500km",
       fout=paste0("paperfig_anticyccorr",ind,"_3reanals1910_proj100_rad10cv0.075_500km_fieldsig2.pdf"))

plot_indcorr_panel(1910,2014,seasons=rbind(c(5,10),c(11,4)),snames=c("MJJASO","NDJFMA"),
       dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",iname=ind,
       type="cyclone",proj="proj100_rad5cv0.15",type2="_500km",
       fout=paste0("paperfig_cyccorr",ind,"_3reanals1910_proj100_rad5cv0.15_500km_fieldsig2.pdf"))
}
