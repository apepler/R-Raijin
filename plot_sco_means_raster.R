...
fname=paste("POAMA/",proj,"/e24a/globalcyclones_4monthlead_day1_cv",cv,".nc",sep="")
a=nc_open(fname)
...
fname=paste("access-s1/",proj,"/globalcyclones_4monthlead_day1_cv",cv,".nc",sep="")
a=nc_open(fname)
ylist=ncvar_get(a,"year")
members=ncvar_get(a,"members")
Y=which(ylist%in%years)
...
meanfreq=apply(systems,c(1,2,4,5),mean,na.rm=T)*100
rm(tmp)

print(apply(meanfreq,c(3,4),max,na.rm=T))
...
fname=paste("Cycfreq_ACCESSvPOAMA_",sprintf("%2.2d",m),"01_lead0_cv",cv,".pdf",sep="")
...
m1=abind(meanfreq[(length(lon)-5):length(lon),length(lat):1,m,n],meanfreq[,length(lat):1,m,n],meanfreq[1:5,length(lat):1,m,n],along=1)
...
meanfreq=apply(apply(systems,c(1,2,3,5),sum),c(1,2,4),mean,na.rm=T)*100
...
fname=paste("Cycfreq_ACCESSvPOAMA_annual_lead0_cv",cv,".pdf",sep="")
