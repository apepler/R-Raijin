...
if(month2>=month1)
 {
  ...
  systems[,,y,em]=table(factor(fixes$Lon2[I],levels=seq(0,359)),factor(fixes$Lat2[I],levels=seq(-90,89)))
  ...
  if(y==1)
  {
   ...
   systems[,,y-1,em]=table(factor(fixes2$Lon2,levels=seq(0,359)),factor(fixes2$Lat2,levels=seq(-90,89)))
...
if(month2>=month1)
 {
  I=which(fixes$Month>=month1 & fixes$Month<=month2)
  systems2[,,y,em]=table(factor(fixes$Lon2[I],levels=seq(0,359)),factor(fixes$Lat2[I],levels=seq(-90,89)))
  ...
  if(y==1)
  {
   ...
   systems2[,,y-1,em]=table(factor(fixes2$Lon2,levels=seq(0,359)),factor(fixes2$Lat2,levels=seq(-90,89)))
...
for(i in 1:length(lon))
  for(j in 1:length(lat))
  {
     ...
     systems[i,j,,]=apply(systems2[I,J,,],c(3,4),sum,na.rm=T)
  }
...
for(i in 1:length(lon))
  for(j in 1:length(lat))
     ...
     if(meanfreq[i,j,1]>=1 & meanfreq[i,j,2]>=1)
     {
...
if(is.na(fout)) fname=paste("Systems_frqtrend_",year1,"_",year2,"_global2.pdf",sep="") else fname=paste(fout,"_all_freq_global2.pdf",sep="")
...
map('world2',add=T)
contour(lon,lat,cyccorr[,,1,1],levels=breaks,add=T,lwd=2,col="black",drawlabels=F)
dev.off()
...
image(lon,lat,cyccorr[,,s,1],breaks=breaks,col=cols,xlab="",ylab="",
          main=cornames[s])
map('world2',add=T)
contour(lon,lat,cyccorr[,,s,1],levels=breaks,add=T,lwd=2,col="black",drawlabels=F)
