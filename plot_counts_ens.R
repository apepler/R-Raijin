...
plot_counts_ens<-function(year1,year2,dir="gcyc_out",cv=NA,dur=NA,slp=NA,month1=1,month2=12,fout=NA,hilo="low",members=56)
{
...
dimnames(systems)[[5]]=c("Freq","MSLP","CV","Depth","Radius")

for(y in 1:length(years))
  for(em in 1:members)
{
print(paste(years[y],em))
fname=paste(dir,"/tracks_",years[y],"_",em,".dat",sep="")
...
cyctrend<-array(NaN,c(length(lon),length(lat),members+1,5,2))
  ...
  for(j in 1:length(lat))
    ...
    for(em in 1:members)
     if(meanfreq[i,j,em,1]>=5)
     {
      ...
      if(mean(meanfreq[i,j,1:members,1])>=5)
      {
        a=lm(apply(systems[i,j,,1:members,k],1,mean)~years)
        b=summary(a)$coefficients
        if(k==1) cyctrend[i,j,members+1,k,1]=100*a$coefficients[2]/mean(meanfreq[i,j,1:members,1]) else cyctrend[i,j,members+1,k,1]=10*a$coefficients[2]
        cyctrend[i,j,members+1,k,2]=b[2,4]          
      }
...
layout(cbind(c(1,2,3,4),c(5,6,7,8)),width=c(1,0.15))
par(mar=c(3,3,4,1))

if(type[k]=="freq") tmp=apply(meanfreq[,,1:members,k],c(1,2),mean)/100 else tmp=apply(meanfreq[,,1:members,k],c(1,2),mean) # per degree
image(lon,lat,tmp,breaks=breaks1,col=col1,xlab="",ylab="",
          main=paste("Mean:",year1,"-",year2,units1[k]))
map('world2',add=T)

image(lon,lat,cyctrend[,,members+1,k,1],breaks=breaks2,col=col2,xlab="",ylab="",
          main=paste("Trend in ensemble mean:",year1,"-",year2,units2[k]))
map('world2',add=T)
sigmask=which(cyctrend[,,members+1,k,2]<0.05 & apply(meanfreq[,,1:members,k],c(1,2),mean)>=5,arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)

image(lon,lat,apply(cyctrend[,,1:members,k,1],c(1,2),min),breaks=breaks2,col=col2,xlab="",ylab="",
      main=paste("Smallest trend:",year1,"-",year2,units2[k]))
map('world2',add=T)
sigmask=which(apply(cyctrend[,,1:members,k,2]<0.05,c(1,2),sum)>=(0.95*members) & apply(meanfreq[,,1:members,k],c(1,2),mean)>=5,arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)

image(lon,lat,apply(cyctrend[,,1:members,k,1],c(1,2),max),breaks=breaks2,col=col2,xlab="",ylab="",
      main=paste("Largest trend:",year1,"-",year2,units2[k]))
map('world2',add=T)
sigmask=which(apply(cyctrend[,,1:members,k,2]<0.05,c(1,2),sum)>=(0.95*members) & apply(meanfreq[,,1:members,k],c(1,2),mean)>=5,arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)
...
for(s in 1:5)
{
  plot_counts_ens(1950,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/proj100_highs_rad10cv0.075",fout=paste("20CRensemble_UM_anticyclonetrend_1950-2014_rad10cv075_v2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s])
  
  plot_counts_ens(1950,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/proj100_highs_rad10cv0.075",fout=paste("20CRensemble_UM_anticyclonetrend_1950-2014_rad10cv075_slp1020_D2_v2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],slp=1020,dur=2)
  
#  plot_counts_ens(1910,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/proj100_highs_rad10cv0.075",fout=paste("20CRensemble_UM_anticyclonetrend_1910-2014_rad10cv075_slp1020_D2_1910_v2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],slp=1020,dur=2)

  plot_counts_ens(1980,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/proj100_highs_rad10cv0.075",fout=paste("20CRensemble_UM_anticyclonetrend_1980-2014_rad10cv075_slp1020_D2_v2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],slp=1020,dur=2)
