...
if(year1<1979)
{
pdf(file=paste0(fout,"_",year3,year2,"vs",year1,year4,"_fieldsig2.pdf"),width=4*ss,height=6.3)
layout(rbind(seq(1,ss),seq(ss+1,ss*2),rep(ss*2+1,ss)),height=c(1,1,0.3))
} else {
pdf(file=paste0(fout,"_",year3,year2,"vs",year1,year4,"_fieldsig2.pdf"),width=4*ss,height=9)
...
for(n in startN:3)
{
  ...
  for(s in 1:ss)
  {
   ...
   fid=s+ss*(n-startN)
   tit=paste0(letters[fid],") ",snames[s]," % change in ",reanals[n])
