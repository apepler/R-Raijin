...
### Make table of events to combine with DJF for exclusion
 if(!is.na(dur))
  ...
  I=which(fixes$Fix!=1)
  if(I[1]==1) I=I[-1]
  fixes$Move[I]=unlist(lapply(I,function(i) spDistsN1(cbind(fixes$Lon[i],fixes$Lat[i]),cbind(fixes$Lon[i-1],fixes$Lat[i-1]),longlat=T)))

  x<-rle(fixes$ID)
  events<-cbind(x$values,x$lengths,matrix(data=0,nrow=length(x$values),ncol=1))
  events[,3]=aggregate(fixes$Move,by=list(fixes$ID),FUN=sum,na.rm=T)[,2]
...
Y1=which(years>=1960 & years<=1979)
Y2=which(years>=1997 & years<=2016)
...
if(is.na(fout)) fname=paste("Systems_",type[k],"_change_6079_9716_",year1,"_",year2,"_global.pdf",sep="") else 
   fname=paste(fout,"_freqchange_6079_9716_global.pdf",sep="")


pdf(file=fname,width=14,height=5.5)
layout(cbind(c(1,3),c(2,3)),height=c(1,0.15))
...
image(lon,lat,cyctrend[,,s,3],breaks=breaks,col=cols,xlab="",ylab="",
          main=paste("% change in",snames[s],": 1997-2016 vs 1960-1979"))
map('world2',add=T)
...
plot_counts_many(dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_1950-2014_rad10cv075_500kmaccum",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],move=500)

plot_counts_many(dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_lows_rad5cv0.15",fout=paste("UM_cyclonetrend_many_1950-2014_rad5cv15_500kmaccum",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],move=500)
