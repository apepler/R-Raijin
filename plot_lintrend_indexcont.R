...
plot_counts_many<-function(year1,year2,dir="gcyc_out",cv=NA,dur=NA,slp=NA,month1=1,month2=12,fout=NA,hilo="low",move=NA,iname="SAM",ifile=NaN)
{
...
if(month2>=month1)
 {
  ...
  systems[,,y]=table(factor(fixes$Lon2[I],levels=seq(0,350,10)),factor(fixes$Lat2[I],levels=seq(-90,80,10)))
  ...
  if(y==1)
  {
   ...
   systems[,,y-1]=table(factor(fixes2$Lon2,levels=seq(0,350,10)),factor(fixes2$Lat2,levels=seq(-90,80,10)))
...
### Calculate linear trend & SAM component
meanfreq=apply(systems,c(1,2),mean)
trends<-array(NaN,c(length(lon),length(lat),4,2))
dimnames(trends)[[3]]=c("Linear.Trend","Detrended.Reg","Index.trend","Index.cont")
...
for(i in 1:length(lon))
  for(j in 1:length(lat))
     if(meanfreq[i,j]>=5)
     {
      regCoef=lm(index2~years)
      index_detrend=resid(regCoef) # Detrended
      trends[i,j,3,1]=regCoef$coefficients[2]
      trends[i,j,3,2]=summary(regCoef)$coefficients[2,4]

      regCoef=lm(systems[i,j,]~years)
      count_detrend=resid(regCoef) # Detrended
      trends[i,j,1,1]=regCoef$coefficients[2]
      trends[i,j,1,2]=summary(regCoef)$coefficients[2,4]
 
      regCoef=lm(count_detrend~index_detrend)
      trends[i,j,2,1]=regCoef$coefficients[2]
      trends[i,j,2,2]=summary(regCoef)$coefficients[2,4]
 
      trends[i,j,4,1]=trends[i,j,2,1]*trends[i,j,3,1]
      trends[i,j,4,2]=trends[i,j,2,2] # Shows where SAM relationship is significant
...
if(month2==(month1+11)) breaks=c(-100,seq(-10,10,2),100) else breaks=c(-100,seq(-5,5),100)
cols=col_anom(length(breaks)-1)

if(is.na(fout)) fname=paste("Systems_",type[k],"_trend_",year1,"_",year2,"_global.pdf",sep="") else fname=paste(fout,"_global.pdf",sep="")
...
image(lon,lat,trends[,,1,1]*10,breaks=breaks,col=cols,xlab="",ylab="",
          main=paste0("Linear trend in ",hilo,"s/decade: ",year1,"-",year2))
map('world2',add=T)
sigmask=which(trends[,,1,2]<0.05 & meanfreq>=5,arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)

image(lon,lat,trends[,,4,1]*10,breaks=breaks,col=cols,xlab="",ylab="",
          main=paste(iname,"component of trend"))
map('world2',add=T)
sigmask=which(trends[,,4,2]<0.05 & meanfreq>=5,arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)
...
for(s in 6:7)
{
print(paste("Running ERAI",seas[s]))  
plot_counts_many(1980,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_STRIcont_1980-2016_rad10cv075_D2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,iname="STRI")

plot_counts_many(1980,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_STRPcont_1980-2016_rad10cv075_D2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,iname="STRP")

plot_counts_many(1980,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_lows_rad5cv0.15",fout=paste("UM_cyclonetrend_STRIcont_1980-2016_rad5cv15_D2",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,iname="STRI")

plot_counts_many(1980,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_lows_rad5cv0.15",fout=paste("UM_cyclonetrend_STRPcont_1980-2016_rad5cv15_D2",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,iname="STRP")

print(paste("Running NCEP",seas[s]))

plot_counts_many(1950,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/NCEP1/proj100_highs_rad10cv0.075_v2",fout=paste("UM_anticyclonetrend_STRIcont_1950-2016_rad10cv075_D2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,iname="STRI")

plot_counts_many(1950,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/NCEP1/proj100_highs_rad10cv0.075_v2",fout=paste("UM_anticyclonetrend_STRPcont_1950-2016_rad10cv075_D2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,iname="STRP")

plot_counts_many(1950,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/NCEP1/proj100_lows_rad5cv0.15_v2",fout=paste("UM_cyclonetrend_STRIcont_1950-2016_rad5cv15_D2",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,iname="STRI")

plot_counts_many(1950,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/NCEP1/proj100_lows_rad5cv0.15_v2",fout=paste("UM_cyclonetrend_STRPcont_1950-2016_rad5cv15_D2",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,iname="STRP")
