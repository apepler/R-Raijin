...
for(n in startN:3)
{
  ...
  for(s in 1:ss)
  {
   ...
   image(lon,lat,cyctrend[,,1],breaks=breaks,col=col1,xlab="",ylab="",
          main=paste(snames[s],type,"trend:",reanals[n]))
   map('world2',add=T)   
   contour(lon,lat,cyctrend[,,2]<0.05,levels=c(-100,0.05,100),add=T,lwd=2,col="black",drawlabels=F)
  }
}
...
plot_lintrend_panel(1980,2016,seasons=rbind(c(5,10),c(11,4)),snames=c("MJJASO","NDJFMA"),
       dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",
       type="cyclone",proj="proj100_rad5cv0.15",type2="_500km",
       fout="paperfig_cyclintrend8016_3reanals_proj100_rad5cv0.15_500km.pdf")
