...
plot_counts<-function(year1,year2,dir="gcyc_out",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",hcv=NA,lcv=NA,dur=NA,hslp=NA,lslp=NA,month1=1,month2=12,fout=NA,members=56,compdir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/",move=NA)
{
years=seq(year1,year2,1)

lat=seq(-89.5,89.5)
lon=seq(0,359.5)  ### Can always combine into bigger cells later
if(month2>=month1) systems<-array(0,c(length(lon),length(lat),length(years),members+1,2)) else systems<-array(0,c(length(lon),length(lat),length(years)-1,members+1,2))
...
if(em>members) fname=paste(compdir,name[n],"/tracks_",years[y],".dat",sep="") else fname=paste(dir,name[n],"/tracks_",years[y],"_",em,".dat",sep="")
...
if(month2>=month1)
 {
  ...
  systems[,,y,em,n]=table(factor(fixes$Lon2[I],levels=seq(0,359)),factor(fixes$Lat2[I],levels=seq(-90,89)))
 } else {
  if(y==1)
  {
   ...
   systems[,,y-1,em,n]=table(factor(fixes2$Lon2,levels=seq(0,359)),factor(fixes2$Lat2,levels=seq(-90,89)))
...
meanfreq=apply(systems[,,,1:members,],c(1,2,5),mean,na.rm=T)
meanfreq2=apply(systems[,,,members+1,],c(1,2,4),mean,na.rm=T)
### Plot
...
m1=abind(meanfreq[(length(lon)-5):length(lon),length(lat):1,n],meanfreq[,length(lat):1,n],meanfreq[1:5,length(lat):1,n],along=1)
m2=raster(t(m1),xmn=-4.5,xmx=364.5,ymn=-89.5,ymx=89.5)
w2=focalWeight(m2,5,type="circle")
m3=focal(m2,w2)

m1=abind(meanfreq2[(length(lon)-5):length(lon),length(lat):1,n],meanfreq2[,length(lat):1,n],meanfreq2[1:5,length(lat):1,n],along=1)
m2=raster(t(m1),xmn=-4.5,xmx=364.5,ymn=-89.5,ymx=89.5)
w2=focalWeight(m2,5,type="circle")
m3=m3-focal(m2,w2)
...
for(s in c(1,6:7))
{
plot_counts(1980,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",fout=paste("20CRens_UM_meanfreq_vERAI_1980-2014_500km",seas[s],sep=""),month1=m1[s],month2=m2[s],move=500)
