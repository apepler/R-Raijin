...
plot_counts<-function(year1,year2,dir="gcyc_out",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",hcv=NA,lcv=NA,dur=NA,hslp=NA,lslp=NA,month1=1,month2=12,fout=NA,members=56,move=NA)
{
years=seq(year1,year2,1)

lat=seq(-89.5,89.5)
lon=seq(0,359.5)  ### Can always combine into bigger cells later
if(month2>=month1) systems<-array(0,c(length(lon),length(lat),length(years),members,2)) else systems<-array(0,c(length(lon),length(lat),length(years)-1,members,2))
...
fname=paste(dir,name[n],"/tracks_",years[y],"_",em,".dat",sep="")
...
meanfreq=apply(systems,c(1,2,5),mean,na.rm=T)
...
for(s in 1:7)
{
plot_counts(1980,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/20CR/",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",fout=paste("20CRens_UM_meanfreq_1980-2014_500km",seas[s],sep=""),month1=m1[s],month2=m2[s],dur=2,move=500)
