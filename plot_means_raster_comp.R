...
plot_counts<-function(year1,year2,dir="gcyc_out",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",hcv=NA,lcv=NA,dur=NA,hslp=NA,lslp=NA,month1=1,month2=12,fout=NA,move=500)
{
years=seq(year1,year2,1)

lat=seq(-89.5,89.5)
lon=seq(0,359.5)  ### Can always combine into bigger cells later
if(month2>=month1) systems<-array(0,c(length(lon),length(lat),length(years),2,2)) else systems<-array(0,c(length(lon),length(lat),length(years)-1,2,2))
dimnames(systems)[[5]]=c("Cyclones","Anticyclones")
name=c(cyc,anti)
dirs=cbind(paste(dir,cyc,sep=""),paste(dir,anti,sep=""))
cv=c(lcv,hcv)
...
{
print(years[y])
fname=paste(dirs[t,n],"/tracks_",years[y],".dat",sep="")
print(fname)
...
if(month2>=month1)
 {
  ...
  systems[,,y,t,n]=table(factor(fixes$Lon2[I],levels=seq(0,359)),factor(fixes$Lat2[I],levels=seq(-90,89)))
 } else {
  if(y==1)
  {
   ...
   systems[,,y-1,t,n]=table(factor(fixes2$Lon2,levels=seq(0,359)),factor(fixes2$Lat2,levels=seq(-90,89)))
...
if(month1==1 & month2==12) breaks=c(-1000,seq(-3,3,0.5),1000) else breaks=c(-1000,seq(-1,1,0.25),1000)
print("Plotting")
...
for(n in 1:2)
{
m1=abind(meanfreq[(length(lon)-5):length(lon),length(lat):1,1,n],meanfreq[,length(lat):1,1,n],meanfreq[1:5,length(lat):1,1,n],along=1)
m2=raster(t(m1),xmn=-4.5,xmx=364.5,ymn=-89.5,ymx=89.5)
w2=focalWeight(m2,5,type="circle")
m3=focal(m2,w2)

m1=abind(meanfreq[(length(lon)-5):length(lon),length(lat):1,2,n],meanfreq[,length(lat):1,2,n],meanfreq[1:5,length(lat):1,2,n],along=1)
m2=raster(t(m1),xmn=-4.5,xmx=364.5,ymn=-89.5,ymx=89.5)
w2=focalWeight(m2,5,type="circle")
m3=m3-focal(m2,w2)

image(m3,breaks=breaks,col=col1,xlab="",ylab="",
          main=type[n],xlim=c(0,360),ylim=c(-90,90))
map('world2',add=T)
contour(m3,levels=breaks,add=T,lwd=2,col="black",drawlabels=F)
...
for(s in c(1,6:7))
{
#plot_counts(1980,2016,dir="/short/eg3/asp561/cts.dir/gcyc_out/NCEP1/",anti="proj100_highs_rad10cv0.075",cyc="proj100_lows_rad5cv0.15",extra="_v2",fout=paste("NCEP1_UM_meanfreq_vERAI_1980-2016_500km",seas[s],sep=""),month1=m1[s],month2=m2[s],move=500)
plot_counts(1990,2012,dir="/short/eg3/asp561/cts.dir/gcyc_out/",anti=c("ERAI/daily_proj240_highs_rad10cv0.075","ERAI/proj100_highs_rad10cv0.075"),cyc=c("ERAI/daily_proj240_rad5cv0.15","ERAI/proj100_lows_rad5cv0.15"),fout=paste("ERAIdaily_UM_vERAI_meanfreq_1990-2012",seas[s],sep=""),month1=m1[s],month2=m2[s])
