...
pdf(file=fout,width=4*ss,height=(5.4*inum)+0.8)
layout(tmp,height=c(rep(1,2*inum),0.3))
...
par(mar=c(2,2,4,1))
  ...
  for(s in 1:ss)
  {
   ...
   for(ii in 1:inum)
   {
    ...
    if(seasons[s,2]>=seasons[s,1])
    {
      index2[[ii]]=apply(index[K,(seasons[s,1]:seasons[s,2])+1],1,mean)
    } else {
      tmp=cbind(index[K[-length(K)],(seasons[s,1]+1):13],index[K[-1],2:(seasons[s,2]+1)])
      index2[[ii]]=c(apply(tmp,1,mean),NaN)
   }
   }

   cyccorr<-array(NaN,c(length(lon),length(lat),inum*2,2))
   
   plotits=c(paste(snames[s],type,inames[1],"corr:",reanal),
             paste(snames[s],type,inames[1],"pcorr v",inames[2],":",reanal),
             paste(snames[s],type,inames[2],"corr:",reanal),
             paste(snames[s],type,inames[2],"pcorr v",inames[1],":",reanal))
   ...
   for(i in 1:length(lon))
    for(j in 1:length(lat))
     ...
     if(meanfreq[i,j]>=0.1)
     {
      for(ii in 1:inum)
       {
        a=cor.test(freq[i,j,],index2[[ii]],na.rm=T)
        cyccorr[i,j,1+2*(ii-1),1]=a$estimate
        cyccorr[i,j,1+2*(ii-1),2]=a$p.value

        a=cor.test(detrend(freq[i,j,],index2[[inum+1-ii]]),detrend(index2[[ii]],index2[[inum+1-ii]]),na.rm=T)
        cyccorr[i,j,2*ii,1]=a$estimate
        cyccorr[i,j,2*ii,2]=a$p.value
        }
      }

   for(ii in 1:(inum*2))
   {
   image(lon,lat,cyccorr[,,ii,1],breaks=breaks,col=col1,xlab="",ylab="",
          main=paste0(letters[ii],") ",plotits[ii]))
   map('world2',add=T)   
   contour(lon,lat,cyccorr[,,ii,2],levels=c(-100,0.05,100),add=T,lwd=2,col="black",drawlabels=F)
  }
...
plot_indcorr_panel(1982,2016,seasons=smons,
       snames=slist,inames=c("SOI","DMI"),
       dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",reanal="ERAI",
       type="anticyclone",proj="proj100_rad10cv0.075",type2="_500km",ind="SOI",
       fout=paste0("paperfig_anticyccorr_SOIvDMI_ERAIpanel_proj100_rad10cv0.075_500km.pdf"))

plot_indcorr_panel(1982,2016,seasons=smons,
       snames=slist,inames=c("SOI","DMI"),
       dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",reanal="ERAI",
       type="cyclone",proj="proj100_rad5cv0.15",type2="_500km",ind="SOI",
       fout=paste0("paperfig_cyccorr_SOIvDMI_ERAIpanel_proj100_rad5cv0.15_500km.pdf"))
