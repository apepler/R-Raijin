...
plot_counts_many<-function(year1,year2,dir="gcyc_out",name="proj100_highs_rad10cv0.075",cv=NA,dur=NA,slp=NA,month1=1,month2=12,fout=NA,hilo="low",members=56,move=NA)
{
...
if(is.na(fout)) fname=paste("Systems_",type[k],"_trend_",year1,"_",year2,"_global.pdf",sep="") else fname=paste(fout,"_all_",type[k],"_global.pdf",sep="")
...
if(is.na(fout)) fname=paste("Systems_",type[k],"_trend_",year1,"_",year2,"_global.pdf",sep="") else fname=paste(fout,"_mean_",type[k],"_global.pdf",sep="")
...
map('world2',add=T)
sigmask=which(apply(cyctrend[,,,k,2]<0.05,c(1,2),sum)==length(sdirs) & apply(meanfreq[,,,k],c(1,2),mean)>=5 & (apply(cyctrend[,,,k,1]<0,c(1,2),sum)==length(sdirs) | apply(cyctrend[,,,k,1]>0,c(1,2),sum)==length(sdirs)),arr.ind=T)
points(lon[sigmask[,1]],lat[sigmask[,2]],col="black",pch=19,cex=0.6)
...
plot_counts_many(1980,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_1980-2014_rad10cv075_D2_500km",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,move=500)
plot_counts_many(1950,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_1950-2014_rad10cv075_D2_500km",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2,move=500)

plot_counts_many(1980,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_lows_rad5cv0.15",fout=paste("UM_cyclonetrend_many_1980-2014_rad5cv15_D2_500km",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,move=500)
plot_counts_many(1950,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_lows_rad5cv0.15",fout=paste("UM_cyclonetrend_many_1950-2014_rad5cv15_D2_500km",seas[s],sep=""),hilo="low",month1=m1[s],month2=m2[s],dur=2,move=500)  
}
