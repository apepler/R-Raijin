...
plot_freq_panel<-function(year1,year2,season=c(1,12),sname="Annual",
...
pdf(file=paste0(fout,".pdf"),width=4,height=9)
layout(c(1,2,3,4),height=c(1,1,1,0.3))

par(mar=c(2,2,4,1))
freq=array(NaN,c(length(lon),length(lat),length(years),3))
...
for(n in 1:3)
{
  ...
  I=which(years2%in%years)
   ...
   {
    for(y in 1:length(I)) freq[,,y,n]=makesmooth(apply(systems[,,I[y],season[1]:season[2]],c(1,2),sum,na.rm=T))    
   } else {
    tmp=abind(systems[,,I[-length(I)],season[1]:12],systems[,,I[-1],1:season[2]],along=4)
    for(y in 1:(length(I)-1)) freq[,,y,n]=makesmooth(apply(tmp[,,y,],c(1,2),sum,na.rm=T))
   }
}
   meanfreq=apply(freq,c(1,2,4),mean,na.rm=T)
   corrnames=c("ERAIvNCEP","ERAIv20CR","NCEPv20CR")
   corind=cbind(c(1,1,2),c(2,3,1))
   ...
   for(i in 1:length(lon))
    for(j in 1:length(lat))
     if(!is.na(meanfreq[i,j,corind[n,1]]) & !is.na(meanfreq[i,j,corind[n,2]]))
     if(meanfreq[i,j,corind[n,1]]>=0.1 & meanfreq[i,j,corind[n,2]]>=0.1)
     {
      a=cor.test(freq[i,j,,corind[n,1]],freq[i,j,,corind[n,2]],na.rm=T)
      cyccorr[i,j,1]=a$estimate
      cyccorr[i,j,2]=a$p.value
      }

   image(lon,lat,cyccorr[,,1],breaks=breaks,col=col1,xlab="",ylab="",
          main=paste0(letters[n],") ",sname," ",type," corr: ",corrnames[n]))
   map('world2',add=T)
   contour(lon,lat,cyccorr[,,1],levels=c(-100,0.8,100),add=T,lwd=2,col="black",drawlabels=F)
...
seasons=rbind(c(1,12),c(5,10),c(11,4))
 
for(s in 1:3)
{
plot_freq_panel(1980,2014,season=seasons[s,],sname=snames[s],
        dir="/short/eg3/asp561/cts.dir/gcyc_out/netcdf",
        type="anticyclone",proj="proj100_rad10cv0.075",type2="_500km",
        fout=paste0("paperfig_anticyccorr_3reanals_proj100_rad10cv0.075_500km_",snames[s]))
}
