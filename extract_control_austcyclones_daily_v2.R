...
cyclones<-array(0,c(length(lon),length(lat),length(years),length(months),length(indays),numdays))
...
## Need the second year for overlaps
fname2=paste(dir,"/tracks_",years[y]+1,".dat",sep="")
read.table(fname, sep="",skip=1)->fixes2
colnames(fixes2)=c("ID","Fix","Date","Time","Open","Lon","Lat","MSLP","CV","Meh")
fixes2$Year=floor(fixes2$Date/10000)
if(length(unique(fixes2$Year))>1) fixes=rbind(fixes,fixes2[fixes2$Year==unique(fixes2$Year)[2],]) else fixes=rbind(fixes,fixes2)
...
{
indate=paste(years[y],sprintf("%02d",m),sprintf("%02d",indays[ii]),sep="")
...
for(nn in which(!is.na(jlat))) cyclones[ilon[nn],jlat[nn],y,m,ii,idate[nn]]=as.numeric(tmp2[nn,4])
}
}
...
cyc_def <- ncvar_def("cyclones","count",list(dimX,dimY,dimT1,dimT2,dimT3,dimM),fillvalue,"Strongest cyclone identified per day in the Australian region",prec="single")
...
ACCESS_ECLs_4month(1990,2012,indays=c(1,9,17,25),numdays=40,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/daily_proj240_lows_rad5cv0.15/",outfile="ERAIdaily_globalcyclones_proj240_rad5cv0.15_40daylead_strongestcyc_allleads.nc",closed=T)

#ACCESS_ECLs_4month(1990,2012,indays=c(1,9,17,25),numdays=40,dir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/500hPa_z/daily_proj240_lows_rad5cv1/",outfile="ERAIdaily_global500hPacyclones_proj240rad5cv1_40daylead_strongestcyc.nc",closed=T)
