...
plot_counts<-function(year1,year2,dirs,names=c("dir1","dir2"),dur=NA,cv=NA,month1=1,month2=12,fout=NA,move=NA,latlim=c(-90,90),lonlim=c(0,360),closed=T)
{
...
dimnames(systems)[[4]]=names

for(n in 1:2)
for(y in 1:length(years))
{
print(years[y])
fname=paste(dirs[n],"/tracks_",years[y],".dat",sep="")
...
meanfreq=apply(systems,c(1,2,4),mean,na.rm=T) # So comparable to other figures

### Plot

type=paste(names,"Frequency (systems/deg^2)")

if(month1==1 & month2==12) breaks=c(0,0.05,seq(0.25,1,0.25),1.5,2,1000) else breaks=c(0,0.05,seq(0.1,1,0.1),1000)
print("Plotting")
col1=col_val(length(breaks)-1)

if(is.na(fout)) fname=paste("Systems_mean_",year1,"_",year2,".pdf",sep="") else fname=paste(fout,".pdf",sep="")

pdf(file=fname,width=8,height=10,pointsize=16)
layout(c(1,2,3),height=c(1,1,0.2))
par(mar=c(2,2,4,1))

for(n in 1:2)
{
image(lon,lat,meanfreq[,,n],breaks=breaks,col=col1,xlab="",ylab="",
          main=type[n],xlim=lonlim,ylim=latlim)
map('world2',add=T)
map("world",add=T)
...
plot_counts(2011,2015,dirs=c("/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj240_lows_rad2cv1","/short/eg3/asp561/cts.dir/gcyc_out/BARRA/proj240_lows_rad2cv1"),names=c("ERAI","BARRA"),
   fout=paste("ERAI_BARRA_UM_meanfreq_2011-2015",seas[s],sep=""),month1=m1[s],month2=m2[s],dur=2,latlim=c(-50,-10),lonlim=c(90,180),closed=T)
