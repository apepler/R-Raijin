...
dimnames(systems)[[5]]=c("Freq","MSLP","CV","Radius","Depth")
...
if(month2>=month1)
 {
  ...
  for(nn in 1:length(tmp2[,1])) systems[ilon[nn],jlat[nn],y,em,2]=as.numeric(tmp2[nn,3])
 } else {
  if(y==1)
  {
   I=which(fixes$Month>=month1 & !is.na(fixes$LonMove))
   store=fixes[I,]
  } else {
   I=which(fixes$Month<=month2 & !is.na(fixes$LonMove))
   ...
   tmp2=aggregate(fixes2$LonMove,by=list(fixes2$Lon2,fixes2$Lat2),FUN=mean)
   jlat=match(tmp2[,2],seq(-90,80,10))
   ilon=match(tmp2[,1],seq(0,350,10))
   for(nn in 1:length(tmp2[,1])) systems[ilon[nn],jlat[nn],y-1,em,2]=as.numeric(tmp2[nn,3])
...
if(month2>=month1)
 {
  ...
  jlat=match(tmp2[,2],seq(-90,80,10))
  ilon=match(tmp2[,1],seq(0,350,10))
  for(nn in 1:length(tmp2[,1])) systems2[ilon[nn],jlat[nn],y,em,2]=as.numeric(tmp2[nn,3])
 } else {
  if(y==1)
  {
   I=which(fixes$Month>=month1 & !is.na(fixes$LonMove))
   ...
   jlat=match(tmp2[,2],seq(-90,80,10))
   ilon=match(tmp2[,1],seq(0,350,10))
   for(nn in 1:length(tmp2[,1])) systems2[ilon[nn],jlat[nn],y-1,em,2]=as.numeric(tmp2[nn,3])
...
cyctrend<-array(NaN,c(length(lon),length(lat),length(snames),3,2))
...
type=c("freq","LonMove")
units1=c("(Systems/deg^2)","(degrees)")
units2=c("(%/year)","(degrees/decade)")
...
for(s in 1)
{
  ...
  plot_counts_many(1950,2014,dir="/short/eg3/asp561/cts.dir/gcyc_out/",name="proj100_highs_rad10cv0.075",fout=paste("UM_anticyclonetrend_many_1950-2014_rad10cv075_D2",seas[s],sep=""),hilo="high",month1=m1[s],month2=m2[s],dur=2) 
