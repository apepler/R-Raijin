...
storm_combo<-function(year,outf=NA,outdir="",fdir,fdirS,cdir,cdirS,tdir,lonlim=c(-180,180),latlim=c(-90,90),winwidF=0,winwidC=0,winwidT=0,cold=T,daily=F,TSenv=T,closed=T,cvthresh=0,len=0,shiftR=T,expandF=F,hdir,winwidH=7,overH=T)
{
...
if(cold)
{
 ...
 {
 tmp=front
 for(i in 1:length(datelist)) front[,,i]=spreadeffect(tmp[,,i],lon=flon,lat=flat,circ=T,winwid=winwidF)
 }
} else {
...
if(winwidF>0) for(i in 1:length(datelist)) ir_front[,,i]=spreadeffect(grid1[,,i],lon=slon,lat=slat,circ=T,winwid=winwidF)
...
if(winwidC>0)
{
 for(i in 1:length(datelist)) cyclone[,,i]=spreadeffect(tmp[,,i],lon=clon,lat=clat,circ=T,winwid=winwidC)
...
if(TSenv)
{
 ...
 {
 tmp=ts
 for(i in 1:length(datelist)) ts[,,i]=spreadeffect(tmp[,,i],lon=tlon,lat=tlat,circ=T,winwid=winwidT)
 }
} else {
...
# And add in the highs!
fname=paste(hdir,"/tracks_",year,".dat",sep="")
read.table(fname, sep="",skip=1)->fixes
colnames(fixes)=c("ID","Fix","Date","Time","Open","Lon","Lat","MSLP","CV","Meh")
fixes$Year=floor(fixes$Date/10000)
if(length(unique(fixes$Year))>1) fixes=fixes[fixes$Year==unique(fixes$Year)[2],]
...
for(i in 1:length(datelist)) highs[,,i]=spreadeffect(grid1[,,i],lon=clon,lat=clat,circ=T,winwid=winwidH)
print("Highs done")
...
## Now use highs to overwrite the other ones, if necessary
## i.e. there can be no front/ts/cyclone if there is a high within 7 degrees
...
tmp<-array(0,c(length(I),length(J),length(daylist)))
...
tmp<-array(0,c(length(I),length(J),length(daylist)))
for(t in 1:length(daylist)) tmp[,,t]=apply(highs[,,datelist2==daylist[t]],c(1,2),max)
highs=tmp
...
storm_combo[storm_combo==0 & highs==1]=8
...
cyc_def <- ncvar_def("storm_type","count",list(dimX,dimY,dimT),fillvalue,"Storm combinations from Catto and Dowdy 2017 - Cyclone Only (1), Front Only (2), Thunderstorm Only (3), Cyclone-Front (4), Cyclone-Thunderstorm (5), Front-Thunderstorm (6), Triple Storm (7), High (8), Other (0)",prec="integer")
...
for(years in 2015)
{
storm_combo(years,winwidC=0,winwidT=1,winwidF=3,daily=T,
...
#     tdir="/g/data/eg3/asp561/CattoData_20052015/",TSenv=F,
     ...
     hdir="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_highs_rad10cv0.075/",winwidH=7,overH=F,
     outdir="/g/data/eg3/asp561/CattoData_20052015/acaciacombos/",
     outf=paste0("stormcombo_cattodowdysimmonds_TSenv_vic_",years,"_daily_v2_highs7deg.nc"))#,
#     lonlim=c(110,160),latlim=c(-45,-10))
}

#years=2015
#for(windows in c(1,2,3,4))
#{
#storm_combo(years,winwidT=windows,winwidF=windows,winwidC=windows,daily=T,
#     fdir="/g/data/eg3/ajd548/vicci/front_data_JC/",cold=T,
#     cdir="/g/data/eg3/ajd548/vicci/cyclone_data_JC/",
#     tdir="/g/data/eg3/asp561/CattoData_20052015/",TSenv=F,
#     tdir="/g/data/eg3/ajd548/vicci/TS_environments/",TSenv=T,
...
#     cdirS="/short/eg3/asp561/cts.dir/gcyc_out/ERAI/proj100_lows_rad5cv0.15/",closed=T,
#     outdir="/g/data/eg3/asp561/CattoData_20052015/acaciacombos/",
#     outf=paste0("stormcombo_cattodowdysimmonds_TSenv_vic_",years,"_daily_testwindow",windows,".nc"))#,
#     lonlim=c(110,160),latlim=c(-45,-10))
#}
